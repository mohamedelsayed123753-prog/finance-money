...
const DATA: Record<string, any> = {
  ...
  en: {
    plans: [
      ...
      { icon: FileText, title: "The preparation of comprehensive economic feasibility studies." },
...
export function FuturePlans() {
  ...
  return (
    <section id="future-plans" className="py-24 bg-[#030712] text-white overflow-hidden" dir={lang === "ar" ? "rtl" : "ltr"}>
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          
          {/* الصورة: انميشن دوران سينمائي بطيء جداً */}
          <motion.div 
          ...
          <div className="lg:col-span-7 space-y-8 order-2 lg:order-1">
            {data.plans.map((item: any, idx: number) => (
              <motion.div 
                ...
                viewport={{ once: true, amount: 0.3 }}
                transition={{ duration: 1.2, delay: 0.6 + (idx * 0.3), ease: "easeOut" }}
                className="flex items-start gap-5"
              >
                <div className="p-2">
                  <item.icon size={24} className="text-[#bfa15f] shrink-0" />
                </div>
                <h4 className="text-lg font-medium text-slate-300 pt-0.5">{item.title}</h4>
              </motion.div>
            ))}
          </div>
